package com.frankcooper.streaming
...
object ConnHelper extends Serializable {
  ...
  //看下是否是这个名称
  lazy val mongoClient = MongoClient(MongoClientURI("mongodb://192.168.200.134:27017/recommender"))
}
...
object StreamingRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 产生评分流
    val ratingStram = kafkaStream.map {
      case msg =>
        var attr = msg.value().split("\\|")
        (attr(0).toInt, attr(1).toInt, attr(2).toDouble, attr(3).toInt)
    }

    ratingStram.foreachRDD {
      rdd =>
        rdd.map {
